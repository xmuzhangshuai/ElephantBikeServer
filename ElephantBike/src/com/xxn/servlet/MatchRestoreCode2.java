		...
		if (null != access_token && servertoken.equals(access_token)) {
			...
			if (NormalUtil.isStringLegal(phone)
				...
				if (resBike != null) {
					...
					String serverpass = PassDemo.getReturnPass(n,
							Integer.parseInt(z), tlist.get(0));
					String _serverpass = PassDemo.getReturnPass(n,
							Integer.parseInt(z), tlist.get(1));
					System.out.println("服务器密码:" + serverpass);
					if (serverpass.equals(clientpass)
							|| _serverpass.equals(clientpass)) {
						// 设置密码，写入数据库
						if (serverpass.equals(clientpass)) {
						...
						if (result == 1) {
							String unlockpass = PassDemo.getUnlockPass(
									resBike.getM() + 1, Integer.parseInt(z));
							map.put(BikeConstants.STATUS, BikeConstants.SUCCESS);
							map.put("pass", unlockpass);
						} else {
							map.put(BikeConstants.STATUS, BikeConstants.FAIL);
							map.put(BikeConstants.MESSAGE,
									"恢复密码正确，恢复密码写入数据库失败，请重试");
						}
					} else {
						String lasspass = resBike.getLastpass();
						System.out.println("client:" + clientpass
								+ "--lasspass:" + lasspass);
						if (NormalUtil.isStringLegal(lasspass)) {
							// 数据库的上一次正确恢复密码不为空
							if (clientpass.charAt(0) == lasspass.charAt(0)
									&& clientpass.charAt(2) == lasspass
											.charAt(2)
									&& clientpass.charAt(4) == lasspass
											.charAt(4)) {
								System.out.println("135位相同");
								String unlockpass = PassDemo.getUnlockPass(
										resBike.getM(), Integer.parseInt(z));
